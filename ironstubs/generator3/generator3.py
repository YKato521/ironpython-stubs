...
def redo_module(module_name, outfile, module_file_name, doing_builtins):
    ...
    if mod:
        action("restoring")
        r = ModuleRedeclarator(
            mod, outfile, module_file_name, doing_builtins=doing_builtins
        )
...
def cut_binary_lib_suffix(path, f):
    ...
    if match:
        ...
        retlen = len(ret)
        if (
            ret.endswith("module") and retlen > modlen and f.endswith(".so")
        ):  # what for?
...
def is_skipped_module(path, f):
    return (
        is_mac_skipped_module(path, f)
        or is_posix_skipped_module(path, f[: f.rindex(".")])
        or "pynestkernel" in f
    )
...
def is_module(d, root):
    return (
        ...
        or os.path.exists(os.path.join(root, d, "__init__.pyc"))
        or os.path.exists(os.path.join(root, d, "__init__.pyo"))
    )
...
def list_binaries(paths):
    ...
    for path in paths:
        if path == os.path.dirname(sys.argv[0]):
            continue
        for root, files in walk_python_path(path):
            ...
            if prefix:
                prefix += "."
            note(
                "root: %s path: %s prefix: %s preprefix: %s",
                root,
                path,
                prefix,
                preprefix,
                ...
                if name and not is_skipped_module(root, f):
                    ...
                    if preprefix:
                        ...
                        if pre_name in res:
                            res.pop(
                                pre_name
                            )  # there might be a dupe, if paths got both a/b and a/b/c
                    ...
                    if name.endswith(".py"):
                        ...
                        if target_path is not None:
                            ...
                            folder_path = os.path.join(
                                target_path, target_dir_path, relpath
                            )
...
def process_one(name, mod_file_name, doing_builtins, subdir, quiet=False):
    ...
    try:
        ...
        if imported_module_names is None:
            imported_module_names = [
                m for m in sys.modules.keys() if m not in old_modules
            ]
        ...
        if imported_module_names and redo_imports:
            for m in sys.modules.keys():
                if m.startswith("pycharm_generator_utils"):
                    continue
                ...
                if (
                    m not in old_modules
                    and m not in imported_module_names
                    and m != name
                    and not hasattr(sys.modules[m], "__file__")
                ):
                    if not quiet:
                        say(m)
                        sys.stdout.flush()
