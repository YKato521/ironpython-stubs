...
version = ((sys.hexversion & (0xFF << 24)) >> 24, (sys.hexversion & (0xFF << 16)) >> 16)
...
NUM_IDENT_PATTERN = re.compile(
    "([A-Za-z_]+)[0-9]?[A-Za-z_]*"
)  # 'foo_123' -> $1 = 'foo_'
STR_CHAR_PATTERN = "[0-9A-Za-z_.,\+\-&\*% ]"

DOC_FUNC_RE = re.compile(
    "(?:.*\.)?(\w+)\(([^\)]*)\).*"
)  # $1 = function name, $2 = arglist

SANE_REPR_RE = re.compile(
    IDENT_PATTERN + "(?:\(.*\))?"
)  # identifier with possible (...), go catches

IDENT_RE = re.compile("(" + IDENT_PATTERN + ")")  # $1 = identifier

STARS_IDENT_RE = re.compile(
    "(\*?\*?" + IDENT_PATTERN + ")"
)  # $1 = identifier, maybe with a * or **

IDENT_EQ_RE = re.compile(
    "(" + IDENT_PATTERN + "\s*=)"
)  # $1 = identifier with a following '='

SIMPLE_VALUE_RE = re.compile(
    "(\([+-]?[0-9](?:\s*,\s*[+-]?[0-9])*\))|"
    + "([+-]?[0-9]+\.?[0-9]*(?:[Ee]?[+-]?[0-9]+\.?[0-9]*)?)|"  # a numeric tuple, e.g. in pygame
    + "('"  # number
    + STR_CHAR_PATTERN
    + "*')|"
...
tupleStr << (
    Suppress("(")
    + Optional(delimitedList(listItem))
    + Optional(Literal(","))
    + Suppress(")")
).setResultsName("tuple")

listStr << (
    lbrack + Optional(delimitedList(listItem) + Optional(Literal(","))) + rbrack
).setResultsName("list")

dictEntry = Group(listItem + colon + listItem)
dictStr << (
    lbrace + Optional(delimitedList(dictEntry) + Optional(Literal(","))) + rbrace
).setResultsName("dict")
...
ident = Word(alphas + "_", alphanums + "_-.").setName(
    "ident"
)  # we accept things like "foo-or-bar"
decorated_ident = ident + Optional(
    Suppress(SP + Literal(":") + SP + ident)
)  # accept "foo: bar", ignore "bar"
spaced_ident = Combine(
    decorated_ident + ZeroOrMore(Literal(" ") + decorated_ident)
)  # we accept 'list or tuple' or 'C struct'

# allow quoted names, because __setattr__, etc docs use it
paramname = spaced_ident | APOS + spaced_ident + APOS | QUOTE + spaced_ident + QUOTE

parenthesized_tuple = (
    Literal("(")
    + Optional(delimitedList(listItem, combine=True))
    + Optional(Literal(","))
    + Literal(")")
).setResultsName("(tuple)")

initializer = (
    SP + Suppress("=") + SP + Combine(parenthesized_tuple | listItem | ident)
).setName(
    "=init"
)  # accept foo=defaultfoo

param = Group(
    Empty().setParseAction(replaceWith(T_SIMPLE))
    + Combine(Optional(oneOf("* **")) + paramname)
    + Optional(initializer)
)

ellipsis = Group(
    Empty().setParseAction(replaceWith(T_SIMPLE))
    + (Literal("..") + ZeroOrMore(Literal("."))).setParseAction(
        replaceWith(TRIPLE_DOT)
    )  # we want to accept both 'foo,..' and 'foo, ...'
)
...
nestedParamSeq = Group(
    ...
    + simpleParamSeq
    + Optional(ellipsis + Optional(COMMA) + Optional(simpleParamSeq))
    + Suppress(")")
)  # we accept "(a1, ... an)"
...
optionalPart << (
    Group(
        Suppress("[").setParseAction(replaceWith(T_OPTIONAL))
        + Optional(COMMA)
        + paramSeq
        + Optional(ellipsis)
        + Suppress("]")
...
if version[0] >= 3:
    v = OMIT_NAME_IN_MODULE.get(BUILTIN_MOD_NAME, []) + [
        "True",
        "False",
        "None",
        "__debug__",
...
ADD_VALUE_IN_MODULE = {
    "sys": (
        "exc_value = Exception()",
        "exc_traceback=None",
    ),  # only present after an exception in current thread
}
...
PREDEFINED_BUILTIN_SIGS = {  # TODO: user-skeleton
    (
        "type",
        "__init__",
    ): "(cls, what, bases=None, dict=None)",  # two sigs squeezed into one
    ("object", "__init__"): "(self)",
    (
        "object",
        "__new__",
    ): "(cls, *more)",  # only for the sake of parameter names readability
    ...
    (
        None,
        "range",
    ): "(start=None, stop=None, step=None)",  # suboptimal: allows empty arglist
...
if version[0] < 3:
    PREDEFINED_BUILTIN_SIGS[
        ("unicode", "__init__")
    ] = "(self, string=u'', encoding=None, errors='strict')"  # overrides a fake
    PREDEFINED_BUILTIN_SIGS[("super", "__init__")] = "(self, type1, type2=None)"
    PREDEFINED_BUILTIN_SIGS[
        (None, "min")
    ] = "(*args, **kwargs)"  # too permissive, but py2.x won't allow a better sig
    PREDEFINED_BUILTIN_SIGS[(None, "max")] = "(*args, **kwargs)"
    PREDEFINED_BUILTIN_SIGS[
        ("str", "__init__")
    ] = "(self, string='')"  # overrides a fake
    PREDEFINED_BUILTIN_SIGS[
        (None, "print")
    ] = "(*args, **kwargs)"  # can't do better in 2.x
else:
    ...
    PREDEFINED_BUILTIN_SIGS[
        ("str", "__init__")
    ] = "(self, value='', encoding=None, errors='strict')"  # overrides a fake
    PREDEFINED_BUILTIN_SIGS[("str", "format")] = "(self, *args, **kwargs)"
    PREDEFINED_BUILTIN_SIGS[
        ("bytes", "__init__")
    ] = "(self, value=b'', encoding=None, errors='strict')"  # overrides a fake
...
if version == (2, 5):
    PREDEFINED_BUILTIN_SIGS[
        ("unicode", "splitlines")
    ] = "(keepends=None)"  # a typo in docstring there

if version >= (2, 7):
    PREDEFINED_BUILTIN_SIGS[
        ("enumerate", "__init__")
    ] = "(self, iterable, start=0)"  # dosctring omits this completely.
...
PREDEFINED_MOD_CLASS_SIGS = {  # TODO: user-skeleton
    ...
    (datetime_mod, "datetime", "utcfromtimestamp"): (
        "(self, timestamp)",
        "datetime(1, 1, 1)",
    ),
    (datetime_mod, "datetime", "utcnow"): ("(cls)", "datetime(1, 1, 1)"),
    (datetime_mod, "datetime", "utctimetuple"): (
        "(self)",
        "(0, 0, 0, 0, 0, 0, 0, 0, 0)",
    ),
    (datetime_mod, "time", "__new__"): (
        "(cls, hour=None, minute=None, second=None, microsecond=None, tzinfo=None)",
        None,
    ),
    ...
    ("numpy.core.multiarray", None, "arange"): (
        "(start=None, stop=None, step=None, dtype=None)",
        None,
    ),
    # same as range()
    ...
    ("numpy.core.multiarray", "ndarray", "reshape"): (
        "(self, shape, *shapes, order='C')",
        None,
    ),
    ("numpy.core.multiarray", "ndarray", "resize"): (
        "(self, *new_shape, refcheck=True)",
        None,
    ),
}
...
for name in bin_collections_names:
    PREDEFINED_MOD_CLASS_SIGS[(name, "deque", "__init__")] = (
        "(self, iterable=(), maxlen=None)",
        None,
    )
    PREDEFINED_MOD_CLASS_SIGS[(name, "defaultdict", "__init__")] = (
        "(self, default_factory=None, **kwargs)",
        None,
    )

if version[0] < 3:
    PREDEFINED_MOD_CLASS_SIGS[("exceptions", "BaseException", "__unicode__")] = (
        "(self)",
        UNICODE_LIT,
    )
    PREDEFINED_MOD_CLASS_SIGS[("itertools", "product", "__init__")] = (
        "(self, *iterables, **kwargs)",
        LIST_LIT,
    )
else:
    PREDEFINED_MOD_CLASS_SIGS[("itertools", "product", "__init__")] = (
        "(self, *iterables, repeat=1)",
        LIST_LIT,
    )
...
KNOWN_PROPS = {
    ...
    "_struct": {("Struct", "size"): ("r", G_INT), ("Struct", "format"): ("r", G_STR),},
    datetime_mod: {
...
if version[0] < 3:
    ...
    FAKE_BUILTIN_INITS = FAKE_BUILTIN_INITS + (
        getattr(the_builtins, "str"),
        getattr(the_builtins, "bytes"),
    )
...
BIN_MODULE_FNAME_PAT = re.compile(
    r"([a-zA-Z_][0-9a-zA-Z_]*)\.(?:pyc|pyo|(?:(?:[a-zA-Z_0-9\-]+\.)?(?:so|pyd)))$"
)
# possible binary module filename: letter,    alphanum                    architecture per PEP-3149
TYPELIB_MODULE_FNAME_PAT = re.compile(
    "([a-zA-Z_]+[0-9a-zA-Z]*)[0-9a-zA-Z-.]*\\.typelib"
)
